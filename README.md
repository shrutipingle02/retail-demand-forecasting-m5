...
pricing, and calendar data. Four approaches are compared — LightGBM, LSTM/GRU, Seq2Seq
GRU, and a naive baseline — scored on forecasting accuracy against the competition's own
weighted metric.

3,049 products × 10 stores = 30,490 series. Jan 2011 – Apr 2016. 68% of item-days are zero.
...
- No training, no features — the bar every other model has to clear
...
### ✅ Seq2Seq GRU
- One-shot 28-day forecast, no feedback loop
...
- Sales follow a weekly cycle that the naive forecast copies for free. The GRU flattens
  toward the average — right level, wrong shape — because each prediction is fed back in
  as input, so errors compound across the horizon.
...
- LightGBM's first score, 0.0088, was impossible — a data leak, not a result. See below.
